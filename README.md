...
- опционально создаёт приветственный листок на базе шаблона `New User.odt`,
- опционально предлагает отправить созданный приветственный листок на печать по умолчанию,
- ведёт подробный лог и умеет работать в режиме **DRY RUN** (без фактического создания пользователей).

### Шаблон приветственного листка (ODT)

Скрипт ищет шаблон по пути `<project_root>/New User.odt`.

Для надёжной подстановки рекомендуется использовать плейсхолдеры в тексте шаблона:
...
Если плейсхолдеров нет, используется fallback-поиск по префиксам строк в абзацах:
...
Готовые документы сохраняются в `%APPDATA%\ADHelper\generated` с уникальным именем вида
`Welcome_<samAccountName>_<YYYYMMDD_HHMMSS>.odt`.
